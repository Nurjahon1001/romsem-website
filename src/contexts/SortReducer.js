...
const SortReducer = (state, action) => {
  const { setProductsData } = useContext(CartContext);
  const {SORT_BY_TITLE, ASCEND, DESCEND, SORT_BY_SLICE} = ActionTypes
  switch (action.type) {
    case SORT_BY_TITLE: {
    ...
    case ASCEND: {
      const ascendedProducts = defaultProducts.sort(
        (a, b) => b.price - a.price
      );
      setProductsData(ascendedProducts);
      break;
    }
    case DESCEND: {
      const descendedProducts = defaultProducts.sort(
        (a, b) => a.price - b.price
      );
